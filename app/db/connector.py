...
-------------------------------------------------------------------------
quiz_grades   { student_id: str, course_id: str, score, correct, total, analysis,
                improvement_plan, created_at }
...
def get_student_report(student_id: str) -> dict:
    ...
    for cid, c in courses_by_id.items():
        ...
        attempts = _build_quiz_attempts(db, student_id, cid)
...
def get_weakest_course_topic(student_id: str) -> dict | None:
    """Picks the course this student is doing worst in, straight from
    their own grade/quiz data — no LLM involved. This is what grounds a
    "give me study resources" follow-up without having to re-derive the
    weak topic from a previous chat message's free text.

    Prefers final_grade where one exists; for a not-yet-graded course
    falls back to that course's average quiz score (also out of 10), so
    a student who's only taken quizzes so far still gets a sensible
    answer. Returns None if there's no grade or quiz signal at all yet.
    """
    ...
    for c in report["courses"]:
        if c["final_grade"] is not None:
            candidates.append((c["final_grade"], c))
            continue
        scored_attempts = [a["score"] for a in c["quiz_attempts"] if a.get("score") is not None]
        if scored_attempts:
            candidates.append((sum(scored_attempts) / len(scored_attempts), c))

    if not candidates:
        return None

    candidates.sort(key=lambda pair: pair[0])
    weakest_score, weakest_course = candidates[0]
    return {
        "title": weakest_course["title"],
        "code": weakest_course["code"],
        "score": round(weakest_score, 1),
        "matched": "weakest",
    }


def resolve_study_topic(student_id: str, query: str) -> dict | None:
    """Figures out which course a "give me study resources" message is
    about, without any LLM call:

    1. If the student named one of their OWN enrolled courses in the
       message (by title or course code), use that course directly —
       e.g. "resources for Calculus" or "مصادر لمادة الرياضيات".
    2. Otherwise, fall back to their weakest course by grade/quiz score
       (get_weakest_course_topic), which is what covers a bare follow-up
       like "عايز مصادر" right after "انا ضعيف في ايه؟".

    Returns None if the student has no enrolled courses to match against
    and no grade/quiz signal to fall back on.
    """
    report = get_student_report(student_id)
    if report.get("status") == "success":
        q = (query or "").lower()
        for c in report["courses"]:
            ...
            if (title and title.lower() in q) or (code and code.lower() in q):
                return {"title": title, "code": code, "score": c.get("final_grade"), "matched": "named"}

    return get_weakest_course_topic(student_id)


# ------------------------------------------------------------------ #
# Instructor report
# ------------------------------------------------------------------ #
def _build_quiz_attempts(db, student_id: str, course_id: str) -> list:
    """All quiz attempts (score + AI-generated weakness analysis) for a
    student in a course, oldest first. Shared by the student's own report
    and the instructor's roster view, so both see the exact same data."""
    attempts = list(
        db.quiz_grades.find(
...
def _build_student_note(fg, course_grades: list, last_quiz) -> str:
    ...
    final_grade is a 0-100 scale (percentage); quiz score is a separate
    0-10 scale computed directly from correct/total — the two are not the
    same unit, so they're labeled differently below.
    """
    parts = []
    ...
    if fg is None:
        ...
        diff = fg - avg
        if diff >= 5:
            parts.append(f"Above the class average ({avg:.1f}%) by {diff:.1f} points.")
        elif diff <= -5:
            parts.append(f"Below the class average ({avg:.1f}%) by {abs(diff):.1f} points.")
        else:
            parts.append(f"Right around the class average ({avg:.1f}%).")
    ...
    for c in courses:
        ...
        for r in roster:
            if r["final_grade"] is not None:
                ...
                peers = course_grades
            attempts = _build_quiz_attempts(db, r["student_id"], cid)
            last_quiz = attempts[-1] if attempts else None
...
def get_personal_records(role: str, user_id: str, max_items: int = 20) -> str:
    ...
    try:
        if role == "student":
            ...
            lines.append(f"Average grade: {report['average_grade']}%, Average attendance: {report['average_attendance']}%")
        ...
        if role == "instructor":
            ...
            for c in report["courses"][:max_items]:
                ...
                for r in c["roster"]:
                    grade_txt = f"{r['final_grade']}%" if r["final_grade"] is not None else "no grade yet"
...
def get_chat_history(user_id: str, limit: int = 200) -> list[dict]:
    ...
    for t in traces:
        ...
        messages.append({
            ...
            "experience_id": t.get("experience_id"),
...
def update_chat_trace_feedback(experience_id: str, feedback: str) -> bool:
    """Mirrors a thumbs up/down onto the matching chat_traces row, so a
    restored chat history (after logout/login) shows the feedback that
    was already given instead of resetting the buttons. The self-refining
    ExperienceMemory record is the actual source of truth used for
    learning — this is just so the UI stays consistent on reload."""
    db = get_db()
    if db is None:
        return False
    result = db.chat_traces.update_one(
