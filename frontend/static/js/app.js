    ...
    if (state.user.role === "Student") {
      content.innerHTML = `
        <div class="stat-row" style="margin-bottom:22px;">
          ...
          <div class="stat"><div class="stat-value">${data.average_grade != null ? data.average_grade + "%" : "—"}</div><div class="stat-label">Average grade</div></div>
      ...
      content.innerHTML = `
        <div class="stat-row" style="margin-bottom:22px;">
          ...
          <div class="stat"><div class="stat-value">${data.average_class_grade != null ? data.average_class_grade + "%" : "—"}</div><div class="stat-label">Avg. class grade</div></div>
        </div>
...
function courseRowStudent(c) {
  const gradeHtml = c.final_grade != null
    ? `<span class="grade-pill">${c.final_grade}%</span>`
  ...
  return `
    <div class="card">
      <div class="card-head">
        ...
        <span class="badge neutral">${c.enrolled_count} students · avg ${c.average_grade != null ? c.average_grade + "%" : "—"}</span>
      </div>
      <table>
        <thead><tr><th>Student</th><th>Final grade</th><th>Report</th><th>Quiz history</th></tr></thead>
        <tbody>
          ${c.roster.length ? c.roster.map(r => `
            <tr>
              <td>${escapeHtml(r.name)}</td>
              <td>${r.final_grade != null ? r.final_grade + "%" : "In progress"}</td>
              <td class="report-cell">${escapeHtml(r.report || "")}</td>
              <td>${renderStudentQuizHistory(r.quiz_attempts || [])}</td>
            </tr>
          `).join("") : `<tr><td colspan="4" class="empty-row">No students enrolled yet.</td></tr>`}
        </tbody>
      </table>
    </div>`;
}

function renderStudentQuizHistory(attempts) {
  if (attempts.length === 0) {
    return `<span class="quiz-history-empty">No attempts yet</span>`;
  }
  const rows = attempts.map(a => `
    <div class="quiz-history-row"><span>${a.correct}/${a.total} correct</span><span>${a.score}/10</span></div>
    ${a.analysis ? `<div class="quiz-history-note">${escapeHtml(a.analysis)}</div>` : ""}
  `).join("");
  return `
    <details class="quiz-history-toggle">
      <summary>${attempts.length} attempt${attempts.length === 1 ? "" : "s"}</summary>
      ${rows}
    </details>`;
}
...
function renderChatHistory() {
  ...
  scroll.querySelectorAll(".feedback-btn").forEach(btn => {
    btn.addEventListener("click", () => sendChatFeedback(btn));
  });
}
...
function renderMessage(m, i) {
  ...
  const arabic = isArabic(m.content);
  const renderedContent = renderMarkdown(m.content);

  // Thumbs up/down only make sense for academic_agent answers — that's
  // the only agent currently running through the self-refining loop.
  const feedbackHtml = (m.agent !== "quiz_agent" && m.experience_id)
    ? `<span class="feedback-btns" data-msg-index="${i}">
        <button class="feedback-btn up ${m.user_feedback === "up" ? "active" : ""}"
          data-feedback="up" data-experience-id="${m.experience_id}"
          title="Helpful" aria-label="Mark helpful">👍</button>
        <button class="feedback-btn down ${m.user_feedback === "down" ? "active" : ""}"
          data-feedback="down" data-experience-id="${m.experience_id}"
          title="Not helpful" aria-label="Mark not helpful">👎</button>
      </span>`
    : "";

  return `<div class="msg assistant">
    <div class="bubble ${arabic ? "ar-text" : ""}"
         dir="${arabic ? "rtl" : "ltr"}">${renderedContent}</div>
    <div class="msg-meta">
      ${agentBadge}
      <button
        class="speak-btn"
        data-text="${escapeHtml(m.content)}"
        title="Read aloud"
...
async function sendChatFeedback(btn) {
  ...
  const previous = state.chatHistory[msgIndex]?.user_feedback || null;

  // Optimistic update — feels instant, reverted below if the request fails.
  wrapper.querySelectorAll(".feedback-btn").forEach(b => b.classList.remove("active"));
  btn.classList.add("active");

  try {
    await api("/chat/feedback", {
      method: "POST",
      body: { experience_id: experienceId, feedback },
    });
    if (state.chatHistory[msgIndex]) state.chatHistory[msgIndex].user_feedback = feedback;
  } catch (err) {
    wrapper.querySelectorAll(".feedback-btn").forEach(b => b.classList.remove("active"));
    if (previous) {
      wrapper.querySelector(`[data-feedback="${previous}"]`)?.classList.add("active");
    }
...
function _feedbackBadge(fb) {
  if (fb === "up") return `<span class="feedback-badge up" title="Marked helpful">👍</span>`;
  if (fb === "down") return `<span class="feedback-badge down" title="Marked not helpful">👎</span>`;
  return `<span class="feedback-badge none" title="No feedback given">—</span>`;
}

function _weaknessesText(weaknesses) {
  if (!weaknesses || weaknesses.length === 0) return "";
  return weaknesses.map(w => {
    if (typeof w === "string") return w;
    if (w && typeof w === "object" && w.question) return `Missed: "${w.question}"`;
    return JSON.stringify(w);
  }).join("; ");
}

function _recentExperienceRow(e) {
  const scoreTxt = e.score != null ? `${Math.round(e.score * 100)}%` : "—";
  const weaknessTxt = _weaknessesText(e.weaknesses);
  const query = e.query || "";
  const truncated = query.length > 90 ? query.slice(0, 90) + "…" : query;
  return `
    <div class="recent-exp-row">
      <div class="recent-exp-top">
        <span class="recent-exp-query">${escapeHtml(truncated)}</span>
        <span class="recent-exp-score">${scoreTxt}</span>
        ${_feedbackBadge(e.user_feedback)}
      </div>
      ${weaknessTxt ? `<div class="recent-exp-weak">${escapeHtml(weaknessTxt)}</div>` : ""}
    </div>`;
}

function _selfRefiningCard(label, stats) {
  const strategy = (stats.current_strategy || "").trim();
  const fc = stats.feedback_counts || { up: 0, down: 0, none: 0 };
  ...
  return `
    <div class="card" style="margin-top:16px;">
      ...
      <div class="system-grid">
        ...
        <div class="system-card"><div class="system-value">👍 ${fc.up} · 👎 ${fc.down}</div><div class="system-label">User feedback (${fc.none} unrated)</div></div>
      </div>
      ...
      ${recent.length ? `
        <details class="quiz-history-toggle" style="margin-top:14px;">
          <summary>What's feeding this agent's learning (last ${recent.length})</summary>
          ${recent.map(_recentExperienceRow).join("")}
        </details>` : ""}
    </div>`;
}
